...
try:
    ...
    print(
        "Issue loading Recipes file. File empty or invalid JSON file. Blank Recipes list created."
    )
    recipes = []
except ValueError:
...
try:
    ...
    print(
        "Issue loading Goals file. File empty or invalid JSON file. Goal set to default zero."
    )
    recipe_goals = {"year_goal": 0}
except ValueError:
...
def add_recipe():
    ...
    while True:
        choice = input(
            "Would you like to mark recipe as favorite (yes or no): "
        ).lower()
        if choice == "yes":
            favorite = "★"
        elif choice == "no":
    ...
    while True:
        ...
        ingredient_dict = {"ingredient": ingredient, "amount": amount}
    ...
    while True:
        category = input(
            "Enter category (Side Dish/ Main Course/ Dessert/ Beverage): "
        ).lower()
...
def filter_categories():
    ...
    while True:
        cat_filter = input(
            "Enter category filter (Side Dish/ Main Course/ Dessert/ Beverage): "
        ).lower()
    ...
    filtered_categories = [
        recipe
        for recipe in diplay_all_recipes
        if cat_filter in recipe["category"].lower()
    ]
...
def filter_difficulty():
    ...
    while True:
        difficulty_filter = input(
            "Enter difficulty filter (Easy/ Medium/ Hard): "
        ).lower()
    ...
    filtered_difficulty = [
        recipe
        for recipe in diplay_all_recipes
        if difficulty_filter in recipe["difficulty"].lower()
    ]
...
def filter_time():
    ...
    while True:
        time_filter = input(
            "Enter cook time filter ([1] under 30 min/ [2] under 2 hours/ [3] over 2 hours): "
        )
        if time_filter == "1":
            filtered_cook_time = [
                recipe for recipe in diplay_all_recipes if recipe["cook_time"] < 30
            ]
            break
        elif time_filter == "2":
            filtered_cook_time = [
                recipe for recipe in diplay_all_recipes if recipe["cook_time"] < 120
            ]
            break
        elif time_filter == "3":
            filtered_cook_time = [
...
def rating_sort():
    ...
    rated_recipes = [
        rating for rating in diplay_all_recipes if rating["rating"] != "N/A"
    ]
    sorted_rated = sorted(
        rated_recipes, key=lambda recipe: recipe["rating"], reverse=True
    )
...
def cook_time_sort():
    ...
    sorted_time = sorted(
        diplay_all_recipes, key=lambda recipe: recipe["cook_time"], reverse=True
    )
...
def view_statistics():
    ...
    completed_recipes = [
        completed for completed in recipes if completed["finished_date"] != "N/A"
    ]
    ...
    sorted_rated = sorted(
        rated_recipes, key=lambda rated_item: rated_item["rating"], reverse=True
    )
...
# -----------------------------------------------------------------------
#   option [10] start recipe and complete
# -----------------------------------------------------------------------
def start_recipe():
    ...
    while True:
        try:
            choice = int(input("Select recipe number to start: "))
    ...
    print(f'Starting recipe for {selected_recipe["name"]}')
    ...
    print(f'Recipe "{selected_recipe["name"]}" completed.')
    date_string = datetime_now_stamp()
...
def run_recipe_goal():
    ...
    if 0 < current_goal:
        if current_goal <= total_year_finished:
            ...
            print(
                f"Recipe goal not met yet. Only {total_year_finished} recipes finished this year."
            )
    ...
    while True:
        try:
            recipe_goals["year_goal"] = int(
                input("Enter yearly recipe completion goal: ")
            )
    ...
    with open(f"{date_now} recipes.csv", "w", newline="", encoding="utf-8-sig") as file:
        ...
        for recipe in recipes:
            ...
            ingredients_text = " ; ".join(
                f"{ingred['ingredient']} ({ingred['amount']})"
                for ingred in recipe["ingredients"]
            )
            # ";" divides the texts. can use any character

            instructions_text = " ; ".join(
                instr["instruction"] for instr in recipe["instructions"]
            )
...
def recipe_recommendation():
    ...
    if unfinished_recipes:
        ...
        print(
            f'All recipes are finished. Recommending to try the "{recommended_recipe["name"]}" recipe again'
        )
